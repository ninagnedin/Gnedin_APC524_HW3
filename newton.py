...
class Newton(object):
    ...
    def step(self, x, fx=None,y0=None):
        """Take a single step of a Newton method, starting from x
        If the argument fx is provided, assumes fx = f(x)"""
        if fx is None:
            fx = self._f(x)
        if self._Df is None:
            #If y0 is passed as a none it will get caught in if statement later
            Df_x = F.ApproximateJacobian(self._f, x, self._dx,y0)
        else:
            Df_x=F.AnalyticJacobian(self._Df,x)
        h = N.linalg.solve(N.matrix(Df_x), N.matrix(fx))
        return x - float(h[0]) #Chaged to float(h[0]) to avoid adding matrix to float, switched to -
